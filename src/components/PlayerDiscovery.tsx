...
export const PlayerDiscovery: React.FC<PlayerDiscoveryProps> = ({
  ...
  const filteredGolfers = allAvailableGolfers.filter(u => {
    // In "Find Golfers" tab, user MUST enter a search query before any golfers appear
    if (activeTab === 'find') {
    ...
    // In "My Friends" tab: filter by friendship (and optional search query if provided)
    ...
    <div id="player-discovery-container" className="space-y-4 pb-24">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex-1 grid grid-cols-3 gap-1 bg-white p-1 rounded-xl border border-slate-200 shadow-xs">
          ...
          >
            <Search className="w-3.5 h-3.5" /> Find Golfers
          </button>
      ...
      {activeTab === 'requests' ? (
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search golfers by name, @username, or club..."
                className="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-28 py-2.5 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent shadow-2xs"
              />
              <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-1 text-[10px] font-semibold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-200 pointer-events-none">
          ...
          {/* Golfers List or Search-First Initial State */}
          <div className="space-y-3">
            {activeTab === 'find' && !hasSearchQuery ? (
              <div id="player-search-prompt" className="p-8 text-center bg-white border border-slate-200 rounded-3xl shadow-xs space-y-4">
                ...
                <div className="space-y-1.5 max-w-sm mx-auto">
                  <h4 className="text-base font-black text-slate-900">Find & Connect with Golfers</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Search by name, @username, or golf club to discover playing partners across all clubs and send friend requests.
                  </p>
                </div>
                {onOpenAddGolferModal && (
                  <button
              ...
              <div className="p-8 text-center bg-white border border-slate-200 rounded-2xl shadow-xs space-y-2">
                <p className="text-xs font-semibold text-slate-600">
                  {activeTab === 'my_friends' 
                    ? (hasSearchQuery ? `No friends found matching "${searchQuery}".` : 'You have not added any friends to your roster yet.')
                    : isSearchingGlobal
                      ? 'Querying global Supabase directory...'
                      : `No golfers found matching "${searchQuery}".`}
                </p>
                <p className="text-[11px] text-slate-400">
                  {activeTab === 'my_friends' && !hasSearchQuery
                    ? 'Use the Find Golfers tab to search and connect with players.'
                    : 'Check the spelling or try searching by club name or username.'}
                </p>
              </div>
            ) : (
